    ...
    function checkForNameTypo(leftPart, firstname, lastname) {
        ...
        function normalizeName(name) {
            ...
            if (nonLatinRegex.test(name)) {
                // taken from http://semplicewebsites.com/removing-accents-javascript
                ...
                name = name.replace(nonLatinRegex, function(x) { return latinMap[x] || x; })
            }
            return name.replace(initialsRegex, '').toLowerCase().replace(/[^a-z]/g, '');
        }
        ...
        for (var i = 0; i < longerName.length - (shorterName.length - 1); i++) {
            if (checkForCloseMatch(shorterName, longerName.substring(i, shorterName.length + 1 + i))) 
                return null;
        }
        ...
        if (fnameResult && fnameResult.elem.orig == nicknames[fnameResult.elem.correction])
            fnameResult = null;
